...
public class AuthController {
    ...
    private String generateTempPassword() {
        // Exemple simple : cha�ne de 12 caract�res alphanum�riques
        int length = 12;
    ...
    public ResponseEntity<?> forgotPassword(@RequestBody Map<String, String> request) {
        ...
        // G�n�re le mot de passe temporaire et l'envoie par email
        String tempPassword = emailService.sendTemporaryPassword(email);

        // Encode et met � jour le mot de passe dans la base
        BCryptPasswordEncoder encoder = new BCryptPasswordEncoder();
        user.setPassword(encoder.encode(tempPassword));
        userRepository.save(user);

        // Optionnel : si tu pr�f�res s�parer la g�n�ration et l'envoi, tu peux utiliser sendPasswordResetEmail
        // emailService.sendPasswordResetEmail(email, tempPassword);

        // Retourne le mot de passe temporaire dans la r�ponse JSON
    ...
    public ResponseEntity<?> resetPassword(@RequestBody Map<String, String> request) {
        ...
        return ResponseEntity
                ...
                .body(Map.of("message", "Mot de passe mis � jour avec succ�s"));
